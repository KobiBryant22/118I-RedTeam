...
if not os.path.exists(CSV_PATH):
  ...
  with st.form("issue_report"):
    ...
    issue_type = st.selectbox(
      "Issue type:",
      ["Litter", "Damaged Equipment", "Graffiti", "Other"],
    )
    parks = schedule_df["park_name"].sort_values().unique()
    issue_park = st.selectbox("Which park?", parks)

    issue_image = st.file_uploader(
      "Upload a picture of the issue (optional)",
      type=["png", "jpg", "jpeg"],
    )

    submitted = st.form_submit_button("🛠️ Submit Report")
    ...
    if not issue_description.strip():
      st.error("Please describe the issue before submitting.")
        ...
        if issue_image is not None:
            ts    = int(time.time())
            ext   = issue_image.name.rsplit(".", 1)[-1]
            fname = f"{issue_park.replace(' ', '_')}_{ts}.{ext}"
            outp  = os.path.join(REPORTS_DIR, fname)
            with open(outp, "wb") as f:
                f.write(issue_image.getbuffer())
            image_filename = fname
            st.image(issue_image, caption="Preview of your uploaded image")
        new_row = {
